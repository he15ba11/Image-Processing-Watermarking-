...
% Last Modified by GUIDE v2.5 29-Dec-2022 19:53:05
...
% handles    structure with handles and user data (see GUIDATA)
global x;
[fileName, path]=uigetfile({'*.jpg', 'JPG Images';'*.png', 'PNG Images'; '*.tif', 'TIF Images'} ,'select an image');
if fileName ==0
    disp('user canceled');
else
...
% handles    structure with handles and user data (see GUIDATA)
global w;
[fileName, path]=uigetfile({'*.jpg', 'JPG Images';'*.png', 'PNG Images'; '*.tif', 'TIF Images'; '*.bmp', 'BMP Images'; '*.jpeg', 'JPEG Images'} ,'select an image');
if fileName ==0
    disp('user canceled');
else
    
    w=imread(fullfile(path, fileName));
...
y = embed(x, w, key,handles);

function y = embed(x, watermark, key,handles)
...
y = cat(3, I, Cb, Cr);
y = ycbcr2rgb(y);
axes(handles.original_watermark);
 imshow(y);
...
global x;
global w;
z = embedgray(x, w, handles);
function z = embedgray(i, w, handles)

original = rgb2gray(i);
watermark = im2bw(w);

original = imresize(original,[960 960]);
watermark = imresize(watermark,[120 120]);
